...
$(function () {
  chrome.storage.sync.get("hide-user-comment", (storage) => {
    ...
    for (const userConfig of userConfigs) {
      $("#userList").prepend(
        userButton
          .replaceAll("[[userAvatar]]", userConfig.upic)
          .replaceAll("[[userName]]", userConfig.uname)
          .replaceAll("[[userMid]]", userConfig.mid)
      );
...
function addUser(ele, mid, uname, upic) {
  chrome.storage.sync.get("hide-user-comment", (storage) => {
    ...
    chrome.storage.sync.set({ "hide-user-comment": userConfigs }, () => {
      $("#userList").prepend(
        userButton
          .replaceAll("[[userAvatar]]", upic)
          .replaceAll("[[userName]]", uname)
          .replaceAll("[[userMid]]", mid)
      );

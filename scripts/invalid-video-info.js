 ...
 * 结果缓存在 chrome.storage.local：部分结果 30 分钟后重试，空结果 12 小时内不重试，最长保留 30 天。
 *
...
chrome.storage.sync.get(['biliplus-enable', 'invalid-video-info'], storage => {
  ...
  function pruneCache() {
    chrome.storage.local.get(null, all => {
      const now = Date.now();
      const archiveEntries = Object.entries(all).filter(([key]) => key.startsWith(INVALID_META_PREFIX));
      const staleKeys = archiveEntries.filter(([, value]) => {
  ...
  function clickCancelJump() {
    ...
    const nodes = [box, ...box.querySelectorAll('.action-btn, button, a, span, div')];
  ...
  async function handleVideoPage() {
    ...
    removePanel();
    const invalidSignal = pageLooksInvalid() || !!document.querySelector('.go-home-from-404, .error-page .error-panel');
    if (!invalidSignal) {
      stopHomeJumpGuard();
      return;
    }
  ...
  function onRouteChange() {
    ...
    if (isVideoPage()) {
      ...
      const initiallyInvalid = pageLooksInvalid() || !!document.querySelector('.go-home-from-404, .error-page .error-panel');
      handleVideoPage().catch(e => console.error('BILIPLUS invalid-video-info:', e));
      if (!initiallyInvalid) {
        // SPA 错误页可能晚于 document_idle 挂载；延迟复查只看 DOM，不为正常视频额外请求 view。
        videoRetryTimer = setTimeout(() => {
          if (location.href === href && isVideoPage() && (pageLooksInvalid() || document.querySelector('.go-home-from-404, .error-page .error-panel'))) {
            handleVideoPage().catch(e => console.error('BILIPLUS invalid-video-info:', e));
          }
        }, 1200);
      }

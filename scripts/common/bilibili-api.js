class _BILIAPI {
  ...
  */
  ...
  static async getVideoInfo(bvid) {
    const jsonData = await this.proxyFetch(`${_BILIAPI.BILIBILI_API}/x/web-interface/view?bvid=${bvid}`);
    if (!jsonData) throw new Error();
    return jsonData.data;
  }

  static async getOnlineTotal(aid, cid, bvid) {
    const jsonData = await this.proxyFetch(`${_BILIAPI.BILIBILI_API}/x/player/online/total?aid=${aid}&cid=${cid}&bvid=${bvid}`);
    if (!jsonData) throw new Error();
    return jsonData.data;
  }

  static async getUserInfoByKeyword(keyword) {
    const jsonData = await this.proxyFetch(`${_BILIAPI.BILIBILI_API}/x/web-interface/wbi/search/type?search_type=bili_user&keyword=${keyword}`);
    if (!jsonData) throw new Error();
  ...
  static async getNavUserInfo() {
    const jsonData = await this.proxyFetch(`${_BILIAPI.BILIBILI_API}/x/web-interface/nav`);
    if (!jsonData) throw new Error();
    return jsonData.data;
  }
  ...
  static async getAIConclusion(params) {
    ...
    const jsonData = await this.proxyFetch(`${_BILIAPI.BILIBILI_API}/x/web-interface/view/conclusion/get?${query}`);
    if (!jsonData) throw new Error();
    return jsonData.data;
  }
}

class _UTILS {
  ...
  static getMixinKey = (orig) => this.mixinKeyEncTab.map(n => orig[n]).join('').slice(0, 32)
  // 为请求参数进行 wbi 签名
  ...
  static async getWbiKeys() {
    const { wbi_img: { img_url, sub_url } } = await _BILIAPI.getNavUserInfo()
    return {
  ...
  // 获取 ai 总结
  static async getAiConclusion(param) {
    const query = await this.getwts(param)
    const aiConclusion = await _BILIAPI.getAiConclusion(query)
    return aiConclusion
  }

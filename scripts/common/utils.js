class _UTILS {
  ...
  static normalizeCoverUrl(url) {
    ...
    try {
      ...
      const isBiliplusArchive =
        parsed.protocol === 'https:' &&
        parsed.hostname.toLowerCase() === 'img.biliplus.com' &&
        !parsed.username &&
        !parsed.password &&
        /\/bfs\/(archive|storyframe)\//i.test(parsed.pathname);
      if (isBiliplusArchive) {
        // biliplus 的归档图与 B 站 CDN 使用同一路径。改写回 hdslb，
        // 既能恢复这类封面，也不会让页面额外加载第三方图片域名。
        parsed.hostname = 'i0.hdslb.com';
        parsed.port = '';
        value = parsed.toString();
      }
    } catch {
      // 留给 isGoodCoverUrl 统一判定非法 URL。
    }
    return value;
  }
